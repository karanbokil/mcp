...
class TestImagePullFailureDetectionExtended(unittest.TestCase):
    ...
    async def test_detect_image_pull_failures_no_task_definitions(self, mock_validate_images, mock_find_task_defs):
        ...
        self.assertIn('Check if your task definition is named differently', result['recommendations'][0])
    ...
    @patch('awslabs.ecs_mcp_server.api.troubleshooting_tools.detect_image_pull_failures.get_task_definitions')
    @patch('awslabs.ecs_mcp_server.api.troubleshooting_tools.detect_image_pull_failures.validate_container_images')
    async def test_detect_image_pull_failures_all_images_valid(self, mock_validate_images, mock_find_task_defs):
        """Test detect_image_pull_failures when all images are valid."""
        # Mock the task definitions
        mock_find_task_defs.return_value = [
            {
                'taskDefinitionArn': 'arn:aws:ecs:us-west-2:123456789012:task-definition/valid-task-def:1',
        ...
        mock_validate_images.return_value = [
            {
                'image': 'valid-repo/valid-image:latest',
                'task_definition': 'arn:aws:ecs:us-west-2:123456789012:task-definition/valid-task-def:1',
                'container_name': 'web',
                'exists': 'true',
    ...
    @patch('awslabs.ecs_mcp_server.api.troubleshooting_tools.detect_image_pull_failures.get_task_definitions')
    @patch('awslabs.ecs_mcp_server.api.troubleshooting_tools.detect_image_pull_failures.validate_container_images')
    async def test_detect_image_pull_failures_ecr_image_not_found(self, mock_validate_images, mock_find_task_defs):
        """Test detect_image_pull_failures when an ECR image is not found."""
        # Mock the task definitions
        mock_find_task_defs.return_value = [
            {
                'taskDefinitionArn': 'arn:aws:ecs:us-west-2:123456789012:task-definition/ecr-task-def:1',
                'family': 'ecr-task-def',
                'containerDefinitions': [
                    {
                        'name': 'web',
        ...
        mock_validate_images.return_value = [
            {
                'image': '123456789012.dkr.ecr.us-west-2.amazonaws.com/missing-repo:latest',
                'task_definition': 'arn:aws:ecs:us-west-2:123456789012:task-definition/ecr-task-def:1',
                'container_name': 'web',
                'exists': 'false',
    ...
    @patch('awslabs.ecs_mcp_server.api.troubleshooting_tools.detect_image_pull_failures.get_task_definitions')
    @patch('awslabs.ecs_mcp_server.api.troubleshooting_tools.detect_image_pull_failures.validate_container_images')
    async def test_detect_image_pull_failures_external_image_unknown(self, mock_validate_images, mock_find_task_defs):
        """Test detect_image_pull_failures when an external image has unknown status."""
        # Mock the task definitions
        mock_find_task_defs.return_value = [
            {
                'taskDefinitionArn': 'arn:aws:ecs:us-west-2:123456789012:task-definition/external-task-def:1',
        ...
        mock_validate_images.return_value = [
            {
                'image': 'docker.io/unknown-repo/unknown-image:latest',
                'task_definition': 'arn:aws:ecs:us-west-2:123456789012:task-definition/external-task-def:1',
                'container_name': 'web',
                'exists': 'unknown',
                'repository_type': 'external'
            }
    ...
    @patch('awslabs.ecs_mcp_server.api.troubleshooting_tools.detect_image_pull_failures.get_task_definitions')
    @patch('awslabs.ecs_mcp_server.api.troubleshooting_tools.detect_image_pull_failures.validate_container_images')
    async def test_detect_image_pull_failures_other_image_issue(self, mock_validate_images, mock_find_task_defs):
        """Test detect_image_pull_failures when an image has other issues."""
        # Mock the task definitions
        mock_find_task_defs.return_value = [
            {
                'taskDefinitionArn': 'arn:aws:ecs:us-west-2:123456789012:task-definition/issue-task-def:1',
        ...
        mock_validate_images.return_value = [
            {
                'image': 'problem-image:latest',
                'task_definition': 'arn:aws:ecs:us-west-2:123456789012:task-definition/issue-task-def:1',
                'container_name': 'web',
                'exists': 'false',
    ...
    @patch('awslabs.ecs_mcp_server.api.troubleshooting_tools.detect_image_pull_failures.get_task_definitions')
    @patch('awslabs.ecs_mcp_server.api.troubleshooting_tools.detect_image_pull_failures.validate_container_images')
    async def test_detect_image_pull_failures_missing_execution_role(self, mock_validate_images, mock_find_task_defs):
        ...
        mock_find_task_defs.return_value = [
            {
                'taskDefinitionArn': 'arn:aws:ecs:us-west-2:123456789012:task-definition/no-role-task-def:1',
        ...
        mock_validate_images.return_value = [
            {
                'image': '123456789012.dkr.ecr.us-west-2.amazonaws.com/valid-repo:latest',
                'task_definition': 'arn:aws:ecs:us-west-2:123456789012:task-definition/no-role-task-def:1',
                'container_name': 'web',
                'exists': 'true',
                'repository_type': 'ecr'
            }
