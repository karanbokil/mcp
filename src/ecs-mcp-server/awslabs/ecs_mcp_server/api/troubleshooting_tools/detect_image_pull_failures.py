...
async def detect_image_pull_failures(app_name: str) -> Dict[str, Any]:
    ...
    try:
        ...
        for task_def in task_definitions:
            ...
            if not execution_role_arn and any('ecr' in container.get('image', '') for container in task_def.get('containerDefinitions', [])):
                response["recommendations"].append(
                    f"Task definition '{task_def_name}' uses ECR images but does not have an execution role. "
                    "Add an executionRole with AmazonECR-ReadOnly permissions."
                )
