...
async def get_network_data(app_name: str, vpc_id: Optional[str] = None, cluster_name: Optional[str] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        if cluster_name:
            ...
            cluster_response = await handle_aws_api_call(
                ecs.list_clusters,
                {"clusterArns": []}
            )
        ...
        if not vpc_ids:
            ...
            vpc_response = await handle_aws_api_call(
                ec2.describe_vpcs,
                {"Vpcs": []}
            )
        ...
        data = {
            ...
            "raw_resources": {
                ...
                "security_groups": await get_ec2_resource(ec2, 'describe_security_groups', vpc_ids=vpc_ids),
                "route_tables": await get_ec2_resource(ec2, 'describe_route_tables', vpc_ids=vpc_ids),
                "network_interfaces": await get_ec2_resource(ec2, 'describe_network_interfaces', vpc_ids=vpc_ids),
                "nat_gateways": await get_ec2_resource(ec2, 'describe_nat_gateways', vpc_ids=vpc_ids),
                "internet_gateways": await get_ec2_resource(ec2, 'describe_internet_gateways', vpc_ids=vpc_ids),
                
                # ELB resources
                "load_balancers": await get_elb_resources(elbv2, 'describe_load_balancers', vpc_ids),
    ...
    try:
        ...
        for cluster in clusters:
            ...
            tasks = await handle_aws_api_call(
                ...
                {"tasks": []},
                cluster=cluster, 
                tasks=tasks_response['taskArns'][:100]  # Limit to 100 tasks
            )
                ...
                for tag_desc in tags_response.get('TagDescriptions', []):
                    for tag in tag_desc.get('Tags', []):
                        if (tag.get('Key') == 'Name' and 
                            app_name.lower() in tag.get('Value', '').lower()):
                            vpc_id = lb.get('VpcId')
                            if vpc_id:
                                vpc_ids.append(vpc_id)
                            break
...
async def discover_vpcs_from_cloudformation(app_name: str) -> List[str]:
    ...
    try:
        ...
        app_stacks = []
        for stack in stacks:
            if (app_name.lower() in stack.get('StackName', '').lower() and
                stack.get('StackStatus') not in ['DELETE_COMPLETE', 'DELETE_IN_PROGRESS']):
                app_stacks.append(stack.get('StackName'))
        
        # Describe resources in each stack
        for stack_name in app_stacks:
            resources = await handle_aws_api_call(
                cfn.list_stack_resources,
...
async def get_ec2_resource(client, method: str, vpc_ids: List[str] = None, **kwargs) -> Dict[str, Any]:
    """Generic function to call EC2 API methods with VPC filtering when applicable."""
    try:
        filters = []
        if vpc_ids:
            if method in ['describe_subnets', 'describe_security_groups', 
                         'describe_route_tables', 'describe_nat_gateways']:
        ...
        return await handle_aws_api_call(func, error_value, **kwargs)
    except Exception as e:
...
async def get_associated_target_groups(client, app_name: str, vpc_ids: List[str]) -> Dict[str, Any]:
    ...
    try:
        ...
        tg_response = await handle_aws_api_call(
        ...
        # Filter by VPC ID
        if vpc_ids:
            target_groups = [tg for tg in target_groups if tg.get('VpcId') in vpc_ids]
    ...
    return {
        "common_issues": [
            ...
            },
            {
                ...
                "description": "Resources in private subnets may need NAT gateways for outbound access",
                "checks": [
            ...
            },
            {
                ...
                "description": "Services may have name resolution issues with improper DNS settings",
                "checks": [
