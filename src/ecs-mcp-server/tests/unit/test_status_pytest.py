...
async def test_get_deployment_status_active(
    ...
    mock_find_cloudformation_stack.return_value = ("test-app-ecs-infrastructure", {
        ...
        "outputs": {
            "LoadBalancerDNS": "test-app-alb-123456789.us-west-2.elb.amazonaws.com"
        }
    })
    ...
    mock_get_alb_url.assert_called_once_with(
        "test-app",
        ...
        "services": [
            {
                ...
                "events": [
                    {"message": "service test-app-service has reached a steady state."},
                    {"message": "service test-app-service has failed to place a task due to No Container Instances were found in your cluster."}
                ]
            }
        ]
    }
...
@patch("awslabs.ecs_mcp_server.api.status._find_cloudformation_stack")
@patch("awslabs.ecs_mcp_server.api.status.get_aws_client")
async def test_get_deployment_status_service_not_found(mock_get_aws_client, mock_find_cloudformation_stack):
...
async def test_find_cloudformation_stack_not_found(mock_get_aws_client):
    ...
    # Mock get_aws_client
    mock_cfn = MagicMock()
    mock_cfn.describe_stacks.side_effect = Exception("Stack test-app-ecs-infrastructure does not exist")
    mock_get_aws_client.return_value = mock_cfn
...
async def test_get_alb_url(mock_get_aws_client):
    ...
    # Call _get_alb_url
    result = await _get_alb_url(
        app_name="test-app",
        known_stack_name="test-app-ecs-infrastructure"
    )
...
async def test_get_cfn_stack_status(mock_get_aws_client):
    ...
    result = await _get_cfn_stack_status(
        stack_name="test-app-ecs-infrastructure"
    )
    ...
    mock_cfn.describe_stack_events.assert_called_once_with(
        StackName="test-app-ecs-infrastructure"
    )
    ...
    assert result["outputs"]["LoadBalancerDNSName"] == "test-app-alb-123456789.us-west-2.elb.amazonaws.com"
...
def test_get_stack_names_to_try():
    ...
    result = _get_stack_names_to_try(
        app_name="test-app",
        stack_name=None
    )
...
def test_generate_custom_domain_guidance():
    ...
    result = _generate_custom_domain_guidance(
        app_name="test-app",
        alb_url="test-app-alb-123456789.us-west-2.elb.amazonaws.com"
    )
