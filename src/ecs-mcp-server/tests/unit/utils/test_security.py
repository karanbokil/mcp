...
class TestValidateFilePath:
    ...
    def test_directory_traversal_attempts(self, tmp_path):
        ...
        traversal_attempts = [
            ...
            f"{test_file}/../../..",
            f"../../../etc/passwd",
            f"..\\..\\..\\Windows\\System32\\config\\SAM",  # Windows example
        ]
        
        for path in traversal_attempts:
            with pytest.raises(ValidationError) as excinfo:
                validate_file_path(path)
            assert "suspicious traversal patterns" in str(excinfo.value) or "does not exist" in str(excinfo.value)
...
class TestValidateCloudFormationTemplate:
    ...
    def invalid_resources_type_template_file(self, tmp_path):
        ...
        template = {
            "Resources": "This should be an object, not a string"
        }
