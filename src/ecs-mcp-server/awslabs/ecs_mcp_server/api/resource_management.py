...
async def describe_cluster(cluster: str, filters: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        task_count = len(running_tasks_response.get("taskArns", [])) + len(stopped_tasks_response.get("taskArns", []))
...
async def list_tasks(filters: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        failed_count = sum(1 for task in tasks
                          if task.get("lastStatus") == "STOPPED" and
                          task.get("stopCode") != "TaskSucceeded")
...
async def describe_task(task: str, filters: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            "is_failed": (
                task_details.get("lastStatus") == "STOPPED" and 
                task_details.get("stopCode") != "TaskSucceeded"
            ),
...
async def describe_task_definition(task_definition: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if family:
            ...
            if list_response.get("taskDefinitionArns") and list_response["taskDefinitionArns"][0] == task_def.get("taskDefinitionArn"):
                is_latest = True
...
async def describe_container_instance(container_instance: str, filters: Dict[str, Any]) -> Dict[str, Any]:
    """
    ...
    """
    logger.info(f"Describing container instance: {container_instance} in cluster: {filters.get('cluster')}")
